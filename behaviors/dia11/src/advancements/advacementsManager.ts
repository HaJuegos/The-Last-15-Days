...
class AdvancementManager extends TL15DBaseManager {
    ...
    private listOfAdvancements: ListOfAdvs[] = [
        ...
        { textAdv: 'advacement.cautious_armor', tagAdv: 'advCautiousArmor', items: ['ha:cautious_helmet', 'ha:cautious_chestplate', 'ha:cautious_leggings', 'ha:cautious_boots'], isRare: true, allItemsRequired: true },
    ];
    ...
    private actionAdvancements(): void {
        afterEventsSimplified.onEntityDie((args) => {
            ...
            if ((deathEntity && (deathEntity.typeId == vanilla.MinecraftEntityTypes.Ghast || deathEntity.typeId == 'ha:soul_ghast')) && (projectile && projectile.typeId == 'minecraft:fireball') && (sourceEntity && sourceEntity instanceof mc.Player)) {
                this.executeAdvan(sourceEntity, 13);

                if (deathEntity.dimension.id == 'minecraft:overworld') {
                    this.executeAdvan(sourceEntity, 16);
                }
            } else if ((deathEntity && (deathEntity.typeId == vanilla.MinecraftEntityTypes.Ghast || deathEntity.typeId == 'ha:soul_ghast')) && (sourceEntity && sourceEntity instanceof mc.Player)) {
                if (deathEntity.dimension.id == 'minecraft:overworld') {
                    this.executeAdvan(sourceEntity, 16);
                }
            }
