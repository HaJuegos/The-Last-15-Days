...
class EntityEventsManager extends TL15DBaseManager {
    ...
    */
            ...
            if ((hitEntity && sourceEntity) && (sourceEntity.isValid && hitEntity.isValid)) {
                switch (sourceEntity.typeId) {
                    ...
                    case vanilla.MinecraftEntityTypes.Vex: {
                        ...
                        if (variant.value == 1) {
                            hitEntity.addEffect('slowness', worldToolsSimplified.convertSecondsToTicks(60), { amplifier: 9 });
                        }
                    } break;
                }
            }
            ...
            if ((hitEntity && sourceEntity) && (sourceEntity.isValid && hitEntity.isValid)) {
                switch (sourceEntity.typeId) {
                    ...
                    case 'ha:bear_trap': {
                        ...
                        worldToolsSimplified.changeScoreInObj(hitEntity as mc.Player, 'timerBearTrap', 'add', 5);
        ...
        afterEventsSimplified.onProjectileHitEntity((args) => {
            ...
            if ((hitEntity && sourceEntity) && (sourceEntity.isValid && hitEntity.isValid)) {
                ...
                switch (sourceEntity.typeId) {
                    ...
                    case vanilla.MinecraftEntityTypes.SnowGolem: {
                        const variant = sourceEntity.getComponent(mc.EntityComponentTypes.Variant);

                        if (!variant) return;

                        if (variant.value == 1) {
                            hitEntity.tryTeleport(sourceEntity.location);

                            if (hitEntity instanceof mc.Player) {
                                hitEntity.playSound('entity.player.teleport');
                            }
                        }
                    } break;
                }
    ...
    private staticEventsEntity(): void {
        worldToolsSimplified.listenerScriptEvents((args) => {
            ...
            switch (id) {
                ...
                case 'ha:break_blocks': {
                    this.shulkerBallEvents(entity);
                } break;
                case 'ha:nurse_effect': {
                    if (entity instanceof mc.Player) {
                        const health = entity.getComponent(mc.EntityComponentTypes.Health);

                        if (!health) return;

                        const maxH = health.defaultValue;

                        entity.playSound('mob.nurse_npc.purchased');
                        entity.spawnParticle('ha:nurse_effect', { x: entity.location.x, y: entity.location.y + 1, z: entity.location.z });

                        entity.sendMessage({ rawtext: [{ translate: 'dialogue.nurse.regenerated' }] });

                        entity.addEffect('strength', worldToolsSimplified.convertSecondsToTicks(300), { amplifier: 3 });
                        entity.addEffect('resistance', worldToolsSimplified.convertSecondsToTicks(300), { amplifier: 3 });
                        entity.addEffect('regeneration', worldToolsSimplified.convertSecondsToTicks(300), { amplifier: 4 });

                        health.setCurrentValue(maxH / 2);
                    }
                } break;
                case 'ha:nurse_effect_incorrect': {
                    if (entity instanceof mc.Player) {
                        entity.playSound('ui.error_sound');

                        entity.sendMessage({ rawtext: [{ translate: 'dialogue.nurse.error_regenerated' }] });
                    }
                } break;
            }
        });
    ...
    /**
     * Metodo auxiliar encargado de cambiar y romper los bloques cuando un shulker bullet golpea algo.
     * @param {mc.Entity} entity La entidad en cuestion.
     * @returns {void}
     * @author HaJuegos - 11-06-2026
     * @private
     */
    private shulkerBallEvents(entity: mc.Entity): void {
        if (!entity.isValid) return;

        const coords = entity.location;
        const dime = entity.dimension;
        const cx = Math.floor(coords.x);
        const cy = Math.floor(coords.y);
        const cz = Math.floor(coords.z);
        const rad = 3;
        ...
        for (let dx = -rad; dx <= rad; dx++) {
            for (let dy = -rad; dy <= rad; dy++) {
                for (let dz = -rad; dz <= rad; dz++) {
                    ...
                    try {
                        const block = dime.getBlock(blockLoc);

                        if ((block && block.isValid) && !block.isAir) {
                            block.setType(vanilla.MinecraftBlockTypes.Air);
                            dime.spawnParticle('minecraft:knockback_roar_particle', blockLoc);
                            dime.playSound('mob.wither.break_block', blockLoc);
                        }
                    } catch (e) { }
                }
            }
